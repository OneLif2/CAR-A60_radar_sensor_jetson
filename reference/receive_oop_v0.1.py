...
class CANDevice:
        ...
        while self.run & 0x0f:
            if (reclen := self.CANLib.VCI_Receive(self.VCI_USBCAN2, 0, ind, rec, 3000, 100)) > 0:
                for j in range(reclen):
                    ...
                    hex_int_arr = ["0x{:02x}".format(num) for num in int_arr]
    ...
    def openDevice(self):
        ...
        ret = self.CANLib.VCI_InitCAN(self.VCI_USBCAN2, 0, 0, byref(vci_initconfig))
        ...
        ret = self.CANLib.VCI_InitCAN(self.VCI_USBCAN2, 0, 1, byref(vci_initconfig))
        if ret == self.STATUS_OK:
        ...
        ret = self.CANLib.VCI_Transmit(self.VCI_USBCAN2, 0, 1, byref(vci_can_obj), 1)
        if ret == self.STATUS_OK:
            print('CAN1通道发送成功\r\n')
        if ret != self.STATUS_OK:
            print('CAN1通道发送失败\r\n')
